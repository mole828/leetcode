/*
 * @lc app=leetcode id=3633 lang=kotlin
 *
 ...
 * 
 * 
 * landStartTime[i] – the earliest time the i^th land ride can be
 * boarded.
...
class Solution {
    fun earliestFinishTime(landStartTime: IntArray, landDuration: IntArray, waterStartTime: IntArray, waterDuration: IntArray): Int {
        fun landFirst(landIndex: Int, waterIndex: Int): Int {
            ...
            return waterStart + waterDuration[waterIndex]
        }
        fun waterFirst(waterIndex: Int, landIndex: Int): Int {
            val waterFinishTime = waterStartTime[waterIndex] + waterDuration[waterIndex]
            val landStart = maxOf(landStartTime[landIndex], waterFinishTime)
            return landStart + landDuration[landIndex]
        }
        var ans = Int.MAX_VALUE
        for (i in landStartTime.indices) {
            for (j in waterStartTime.indices) {
                ans = minOf(ans, landFirst(i, j))
                ans = minOf(ans, waterFirst(j, i))
            }
        }
        return ans
    }
